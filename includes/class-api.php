...
class Multi_Rating_API {
	...
	public static function display_bokrating_items( $params = array() ) {
		...
		//$rating_items_query = 'SELECT ri.score, ri.overall_notes,ri.reviewcat FROM '. $wpdb->prefix . Multi_Rating::BOKRATING_ITEM_TBL_NAME . ' as ri where ri.post_id = '.$post_id;
		/*$rating_item_rows = $wpdb->get_results( $rating_items_query );
		$helloworld = "";
		foreach ( $rating_item_rows as $rating_item_row ) {
			$helloworld = $helloworld.$rating_item_row->overall_notes;			
		}*/
		$rating_result = array(				
				'post_id' => $post_id,
				'adjusted_star_result' => 4.4,
				'star_result' => 4.4,
				'total_max_option_value' => 5,
				'adjusted_score_result' => 4.4,
				'score_result' => null,
		...
		$rating_review_topics = 'SELECT ri.name, ri.position,ri.reviewcat FROM '. $wpdb->prefix . Multi_Rating::BOKRATING_CAT_TBL_NAME . ' as ri where ri.reviewcat = '.$rating_item_row->reviewcat;
		$rating_item_rows = $wpdb->get_results( $rating_items_query );
		//Get all topics and within each topic the ratings and the star for each rating. 
		*/
		
		ob_start();
		mr_get_template_part( 'bokrating-banner', null, true, array(
